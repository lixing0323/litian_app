<template>
	<view class="login-page">
		...
		<view class="form">
			...
			<view v-if="byCode()" class="form-item">
			...
			<view v-else class="form-item">
				...
				<view class="form-content">
					<input :password="!showPassword" v-model="model.code" class="input" placeholder="请输入密码">
					<text class="uni-icon" :class="{'uni-eye-active': showPassword}" @click="changePassword()">&#xe568;</text>
				</view>
			</view>
		</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			getCode() {
				if (!this.model.tel) return uni.showToast({
					title: '请输入手机号',
					icon: 'none'
				})
				this.sendCode({
