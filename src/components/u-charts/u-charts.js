...
uCharts.prototype.scroll = function(e, srollRate) {
  ...
  if (touches && this.opts.enableScroll === true) {
    ...
    //srollRate滚动倍率
    this.scrollOption.distance = _distance = (validDistance - currentOffset) * (srollRate || 1);
    var scrollDistance = currentOffset + _distance
    //限制两边界
    var dataChartAreaWidth = this.opts.width - this.opts.area[1] - this.opts.area[3];
    var dataChartWidth = this.opts.chartData.eachSpacing * (this.opts.chartData.xAxisData.xAxisPoints.length - 1);
    var maxDistance = Math.abs(dataChartAreaWidth - dataChartWidth)
    if(scrollDistance > 0) scrollDistance = 0
    if((scrollDistance + maxDistance) < 0) scrollDistance = -maxDistance

    var opts = assign({}, this.opts, {
      _scrollDistance_: scrollDistance,
      animation: false
    });
    this.opts = opts;
    drawCharts.call(this, opts.type, opts, this.config, this.context);
...
uCharts.prototype.scrollEnd = function(e) {
  if (this.opts.enableScroll === true) {
    ...
    var scrollDistance = currentOffset + distance
    ...
    this.scrollOption.currentOffset = scrollDistance;
